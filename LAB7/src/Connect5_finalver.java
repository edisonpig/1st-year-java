...
public class Connect5_finalver extends JFrame implements ActionListener {
	JFrame col;
	JPanel board;
	JMenuBar menubar;
	JMenu menu;
	...
	JButton black;
	int won = 0;
	whiteborder r = new whiteborder(1000);

	GridBagConstraints i = new GridBagConstraints();
	BorderLayout l = new BorderLayout();

	public Connect5_finalver() {
	...
	public void printgrid() {
		...
		for (int x = 0; x < column; x++) {
			...
			//	hi[x][y].setBorder(new whiteborder(5));
				hi[x][y].setBackground(Color.white);
		...
		try {
			...
			for (int y = 0;y<=row-1;y++) {
				for (int x = 0; x <= column - 1; x++) {
					...
					if (check[x][y] == 1) {
						hi[x][count[x]].setBackground(pl);
						count[x]--;
					}
					if (check[x][y] == 2) {
						hi[x][count[x]].setBackground(co);
						count[x]--;
					}
				}
				br.readLine();
		...
		for (int x = 0; x < column - 4; x++) {
			for (int y = 0; y < row; y++) {
				...
				if (check[x][y] == 1 && check[x + 1][y] == 1 && check[x + 2][y] == 1 && check[x + 3][y] == 1
						&& check[x + 4][y] == 1) {
					hi[x][y].setBackground(Color.GREEN);
					hi[x + 1][y].setBackground(Color.GREEN);
					hi[x + 2][y].setBackground(Color.GREEN);
					hi[x + 3][y].setBackground(Color.GREEN);
					hi[x + 4][y].setBackground(Color.GREEN);
					won = 1;
					return res = 1;
				}
		...
		for (int x = 0; x < column; x++) {
			for (int y = 0; y < row - 4; y++) {
				if (check[x][y] == 1 && check[x][y + 1] == 1 && check[x][y + 2] == 1 && check[x][y + 3] == 1
						&& check[x][y + 4] == 1) {
					hi[x][y].setBackground(Color.GREEN);
					hi[x][y + 1].setBackground(Color.GREEN);
					hi[x][y + 2].setBackground(Color.GREEN);
					hi[x][y + 3].setBackground(Color.GREEN);
					hi[x][y + 4].setBackground(Color.GREEN);
					won = 1;
					return res = 1;
				}
		...
		for (int x = 0; x <= column - 5; x++) {
			for (int y = 0; y <= row - 5; y++) {
				if (check[x][y] == 1 && check[x + 1][y + 1] == 1 && check[x + 2][y + 2] == 1 && check[x + 3][y + 3] == 1
						&& check[x + 4][y + 4] == 1) {
					hi[x][y].setBackground(Color.GREEN);
					hi[x + 1][y + 1].setBackground(Color.GREEN);
					hi[x + 2][y + 2].setBackground(Color.GREEN);
					hi[x + 3][y + 3].setBackground(Color.GREEN);
					hi[x + 4][y + 4].setBackground(Color.GREEN);
					won = 1;
					return res = 1;
				}
		...
		for (int x = 0; x <= column - 5; x++) {
			for (int y = 0; y <= row - 5; y++) {
				if (check[x][y + 4] == 1 && check[x + 1][y + 3] == 1 && check[x + 2][y + 2] == 1
					...
					hi[x][y + 4].setBackground(Color.GREEN);
					hi[x + 1][y + 3].setBackground(Color.GREEN);
					hi[x + 2][y + 2].setBackground(Color.GREEN);
					hi[x + 3][y + 1].setBackground(Color.GREEN);
					hi[x + 4][y].setBackground(Color.GREEN);
					won = 1;
					return res = 1;
				}
	...
	public static void main(String[] args) {
		JFrame tr = new Connect5_finalver();
